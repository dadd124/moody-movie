...
# 함수의 매게변수가2개 이상인 경우 {람다 함수 활용}
df4 = df.map(lambda a:a)
print(df4.head())
print()


# ---------- 데이터 프레임에 apply 적용 (열/행 전달) ------------
...
def calculate_stats(col):
    ...
    return pd.Series([max_val, max_val, mean_val, median_val], index=['Max', 'Min', 'Mean', 'Median'])
...
result_df3 = df.apply(lambda row: pd.Series([row.max() - row.min(), row.mean()], index=['차이', '평균']), axis=1)
