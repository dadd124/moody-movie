...
# 선형 커널
lin_clf = SVC(kernel='linear', C=1, random_state=42)
# 모델 세팅 # C 높을 수록 예민 (과대적합)
...
print('혼동행렬: ', confusion_matrix(y_test, y_pred_lin))

# RBF 커널 (비선형 경계학습)
# Radial Basis Function (방사 기저 함수)

rbf_clf = SVC(kernel='rbf', C=1.0, gamma=1.0, random_state=42)
...
# 결정경계 시각화 (비교)
def plot_decision_boundary(model, X_sc, title):
    # 그리드 생성
...
# plot_decision_boundary(rbf_clf, X_test_sc, y_test, 'RBF SVC (test)')
print()
