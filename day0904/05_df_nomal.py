...
# horsepower 열의 누락데이터('?') nan 으로 바꾼 뒤 drop 한 다음
...
# label_encoder 로 문자열 범주를 숫자형 번주로 변환
...
preprocessing(type(onehot_reshaped))

onehot_fitted = onehot_encoder.fit_transform(onehot_reshaped)
print(onehot_fitted) # 희소행렬 반환
print()
# print(onehot_fitted.toarry())
print()
# OneHotEncoder(sparse_output=False) 옵션을 주면 바로 어레이로 반환

enconded_df = pd.DataFrame(onehot_fitted.toarray(), columns=onehot_encoder.get_feature_names_out(df['hp_bin']))
print(enconded_df)